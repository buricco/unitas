...
main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "l:u:b:d:p:e:h:k:s:QW:?")) != EOF) {
		...
		case 'b':
			...
			if ((base < 2) || (base > 36)) {
				(void) fprintf(stderr,
		"%s: ERROR: base must be between 2 and 36\n",
					prog);
				exit(1);
			}
			break;
	...
	if (upper) {
		...
		if (ptr == upper) {
			(void) fprintf(stderr,
		"%s: ERROR: invalid upper value specification\n",
				prog);
			exit(1);
		}
	} else
	...
	if (lower) {
		...
		if (ptr == lower) {
			(void) fprintf(stderr,
		"%s: ERROR: invalid lower value specification\n",
				prog);
			exit(1);
		}
	} else
	...
	if (uvalue < lvalue) {
		(void) fprintf(stderr,
		"%s: ERROR: upper value is less than lower value\n",
			prog);
		exit(1);
	}
