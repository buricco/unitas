...
extern	void	cat(char**, size_t *, const char*, const char*);
...
extern	char	*fgetline(char **, size_t *, size_t *, FILE *);
...
extern	char	*getarg(char *, char *);
extern	int	getcomment(char *s, char *q);
extern	int	gethead(int	current, int all);
#define	getline(a, b, c)	ma_getline(a, b, c)
extern	int	getline(char **ptr2line, size_t *max, FILE *f);
...
extern	void	(*setsig(int i, void(*f)(int)))(int);
...
extern	char	*fromS;		/* stored here by sendmail for sendsurg */
extern	size_t	fromSsize;
extern	char	*fromU;		/* stored here by sendmail for sendsurg */
extern	size_t	fromUsize;
...
extern	size_t	hmboxsize;
extern	char	*hmdead;	/* pointer to $HOME/dead.letter */
extern	size_t	hmdeadsize;
...
extern	char	*line;		/* holds a line of a letter in many places */
...
extern	char	*msgtype;
extern	char	*my_name;	/* user's name who invoked this command */
extern	size_t	my_namesize;
...
extern	char	*Rpath;		/* return path to sender of message */
extern	size_t	Rpathsize;
extern	char	rtrnmsg[];	/* "UNDELIVERABLE MAIL being returned by %s" */
extern	int	sav_errno;
extern	char	*savefile;	/* holds filename of save file */
extern	size_t	savefilesize;
extern	void	(*saveint)(int);
extern	char	*seldisp[];
extern	int	sending;	/* TRUE==>sending mail; FALSE==>printing mail */
extern	char	*m_sendto;
extern	size_t	m_sendtosize;
