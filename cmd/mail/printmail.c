...
printmail(void)
{
	...
	while (curlet < nlet) {
		...
		fflush(stdout);
		fflush(stderr);
		if (fgetline(&readbuf, &readbufsize, NULL, stdin) == NULL)
			break;
		resp = readbuf;
 ...
 */
			...
			case 's':
				...
				if (resp[1] == '\n' || resp[1] == '\0') {
					cat(&readbuf, &readbufsize, "s", hmbox);
					resp = readbuf;
				} else if (resp[1] != ' ') {
			...
			case 'r':
				...
				cpy(&m_sendto, &m_sendtosize, resp+k);
				goback(showlet);
				replying = 0;
				setletr(showlet, resp[0]);
				break;
