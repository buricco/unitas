...
#ifdef __svr4__
# include <regexpr.h>
# define LLONG_MAX LONG_MAX /* cheat */
#else
# include <regex.h>
#endif
...
static int getlin(register FILE *f, int *length);
...
static int
getlin(register FILE *f, int *length)
{
    register int        c;
    register char       *p;
    register int        column;
...
#define ret(val) retval=val;done++;break
	...
	case ctrl('B'):
        ...
        case 'f':
            ...
            if (clreol)
                cleareol ();
            printf ("...skipping %lld line", nlines);
            if (nlines > 1)
                pr ("s\n");
            else
                pr ("\n");
...
colon(char *filename, int cmd, off_t nlines)
{
        ...
        case 'f':
                kill_line ();
                if (!no_intty)
                        promptlen = printf ("\"%s\" line %lld",
					    fnames[fnum], Currline);
                else
                        promptlen = printf(
			 "[Not a file] line %lld", Currline);
                fflush (stdout);
                return (-1);
        case 'n':
                if (nlines == 0) {
...
#ifndef __svr4__
static regex_t last_regex;
static int was_last_regex = 0;
#endif
...
#else
        if (regexec(&last_regex, Line, 0, 0, 0))
#endif
                if (--n == 0) {
...
execute (char *filename, char *cmd, ...)
{
	...
	int  count;
		...
		count++;
		if (count > MAXARGS)
			error ("Too many arguments in execute()\n");
...
set_tty(void)
{
	...
	otty.sg_flags &= ~ECHO;
 ioctl(fileno(stderr), TIOCSETP, &otty);
#else
...
reset_tty(void)
{
    ...
    otty.sg_flags |= ECHO;
    otty.sg_flags &= ~CBREAK;
    ioctl(fileno(stderr), TIOCSETP, &ntty);
#else
