...
main(int argc, char **argv)
{
		...
		if (*scan == '+') {
			if (!option_end) {
				if (*++scan == 'm') {
					...
					(void) fprintf(stderr, "tabs: %s: invalid tab spec\n", scan-1);
					usage();
				}
			} else {
				/*
				 ...
				 */
				(void) fprintf(stderr, "tabs: %s: invalid tab stop operand\n", scan);
				usage();
			}
		} else if (*scan == '-') {
			if (!option_end) {
	...
	if (setupterm(terminal, 1, &err) == ERR) {
		(void) fprintf(stderr, "tabs: %s: terminfo file not found\n", terminal);
		usage();
	} else if (!tigetstr("hts")) {
		(void) fprintf(stderr, "tabs: cannot set tabs on terminal type %s\n", terminal);
		usage();
	}
...
scantab(char *scan, int tabvect[NTABS], int level)
{
	...
	if (*scan == '-')
		if ((c = *++scan) == '-')
			...
			endup();
			(void) fprintf(stderr, "tabs: %s: unknown tab code\n", scan);
			usage();
		} else;
	else
	...
	for (i = 0; i < NTABS-1; ) {
		if (*scan == '+') {
			...
			if (t = getnum(&scan))
				...
				endup();
				(void) fprintf(stderr, "tabs: %s: invalid increment\n", scan_save);
				usage();
			}
		} else {
			if ((t = getnum(&scan)) > last)
				...
				endup();
				(void) fprintf(stderr, "tabs: %s: invalid tab stop\n", scan_save);
				usage();
			}
		}
		if (*scan++ != ',') break;
...
filetab(char *scan, int tabvect[NTABS], int level)
{
	...
	if (level) {
		endup();
		(void) fprintf(stderr, "tabs: %s points to another file: invalid file indirection\n",
		scan);
		exit(1);
	}
	if ((fildes = open(scan, O_RDONLY)) < 0) {
		endup();
		(void) fprintf(stderr, "tabs: %s: ", scan);
		perror("");
		exit(1);
	}
...
usage()
{
	(void) fprintf(stderr, "usage: tabs [ -n| --file| [[-code] -a| -a2| -c| -c2| -c3| -f| -p| -s| -u]] \
[+m[n]] [-T type]\n");

	(void) fprintf(stderr, "       tabs [-T type][+m[n]] n1[,n2,...]\n");
