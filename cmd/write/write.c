...
void
usage(int errorlevel)
{
	(void) fprintf (stderr, "Usage: write user_name [terminal]\n");
	exit (errorlevel);
}
...
/*	specific spot and didn't find it, complain and quit.		*/
	...
	if (terminal != NULL && *rterm == '\0') {
		...
		if (strlcat(rterminal, terminal, sizeof (rterminal)) >=
		    sizeof (rterminal)) {
			(void) fprintf(stderr,"Terminal name too long.\n");
			exit(1);
		}
		if (self.ut_pid == 0) {
			if ((passptr = getpwuid(getuid())) == NULL) {
			    (void) fprintf(stderr, "Cannot determine who you are.\n");
			    exit(1);
		    }
...
#else
		(void) fprintf(stderr, "%s is not at \"%s\".\n",
			receipient, terminal);
		exit(1);
#endif	/* UTMP_HACK */
	    }
...
/*	If permissions prevent us from sending to this person - exit	*/
		...
		(void) fprintf(stderr, "%s is not logged on.\n", receipient);
	    exit(1);
	    }
...
/*	utmp file couldn't be found.					*/
	...
	if (!permit1(1))
		(void) fprintf(stderr, "Warning: You have your terminal set to \"mesg -n\"."
		    "  No reply possible.\n");
...
/*	year information at the end of the string.			*/
	...
	time(&tod);
	(void) strftime(time_buf, sizeof (time_buf), "%c", localtime(&tod));
...
/*	Send line to the receipient.					*/
			...
			if (myuid && !permit1(fileno(fp))) {
				(void) fprintf(stderr, "Can no longer write to %s\n", rterminal);
				break;
			}
...
char *command;
{
	...
	if ((child = fork()) == (pid_t)FAILURE)
	    {
	    (void) fprintf(stderr, "Unable to fork.  Try again later.\n");
	    return;
...
openfail()
{
	...
	(void) fprintf(stderr, "Timeout trying to open %s's line(%s).\n",
	    receipient, rterm);
	exit(1);
}
...
char *term;
{
	...
	if (!isatty(fildes)) {
		(void) fprintf(stderr, "%s in utmpx is not a tty\n", term);
