#ifndef lint
static  char sccsid[] = "@(#)pax.c 1.1 94/10/31 SMI"; /* from Mark H. Colburn */
#endif
...
/*
 * uso:
 *   Relative to SUS, the following pax switches appear to be missing (and in
 *   some cases existing switches are incompatible with these):
 *     -k (no clobber)
 *     -n (stop after reading one non-folder entry)
 *     -o (extended options, not defined in SUS)
 *     -t (keep access times the same as before running pax)
 *     -X (do not cross mountpoints when searching for files)
 *   Also relative to Posix.1:2024, the specific nature of -o is specified,
 *   and -H and -L switches for handling directories and links need to be
 *   added.
 * 
 *   No designation whether this pax actually uses the pax variant of tar - it
 *   may be too old for that.  Also, this code needs to be ansified properly.
 */
...
int main(int argc, char **argv)
{
    ...
    /*
     * uso: The original code switched to tar mode if the program was invoked
     *      as ustar, and cpio mode if invoked as paxcpio.  Keep compatibility
     *      with that, but ALSO accept tar and cpio, or anything else that can
     *      end in tar or cpio.
     */
#if 0
    if (strcmp(myname, "ustar")==0) {
	do_tar(argc, argv);
    } else if (strcmp(myname, "paxcpio")==0) {
	do_cpio(argc, argv);
    } else {
	do_pax(argc, argv);
    }
#else
    {
		...
		if (strlen(myname)>2)
		{
			tail=&(myname[strlen(myname)-3]);
			if (!strcmp(tail, "tar"))
			{
				return do_tar (argc, argv);
			}
			if (strlen(myname)>3)
			{
			tail--;
			if (!strcmp(tail, "cpio"))
				return do_cpio (argc, argv);
			}
			return do_pax(argc, argv);
		}
    }
#endif
