...
main(int argc, char **argv)
{
	...
	while (!end_of_file) {
		...
		if ((pwdp = fgetpwent(pwf)) != NULL) {
			if (!file_exist ||
				...
				if (!pwdp->pw_passwd ||
					...
					*pwdp->pw_passwd == 0)) {
					(void) fprintf(stderr,
			"%s: WARNING user %s has no password\n",
					prognamp, sp->sp_namp);
				}
				/*
		...
		if (feof(pwf)) {
			...
			pwerr = 1;
			(void) fprintf(stderr,
			"%s: ERROR: bad entry or blank line at line %d in /etc/passwd\n",
					prognamp, count);
		}
	    }
	} /* end of while */
...
{
	fprintf(stderr,
		"%s: Unexpected failure. Conversion not done.\n", prognamp);
	(void) ulckpwdf();
}
...
{
	fprintf(stderr,
		"%s: Bad entry in /etc/shadow. Conversion not done.\n", prognamp);
	(void) ulckpwdf();
}
