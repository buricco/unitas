...
cell **tab;
{
	...
	if (n != NULL && (p = lookup(n, tab, 0)) != NULL) {
		...
		_dprintf("setsymtab found %o: %s", p, p->nval, NULL);
	...
	p->nextval = tab[h];
	tab[h] = p;
	_dprintf("setsymtab set %o: %s", p, p->nval, NULL);
	_dprintf(" %s %g %o\n", p->sval, p->fval, p->tval);
	return(p);
}
...
register cell *vp;
{
	...
	if (vp->sval == record && donerec == 0)
		recbld();
	_dprintf("getfval: %o", vp, NULL, NULL);
	checkval(vp);
	if ((vp->tval & NUM) == 0) {
...
register cell *vp;
{
	...
	if (vp->sval == record && donerec == 0)
		recbld();
	_dprintf("getsval: %o", vp, NULL, NULL);
	checkval(vp);
	if ((vp->tval & STR) == 0) {
		if (!(vp->tval&FLD))
			strfree(vp->sval);
