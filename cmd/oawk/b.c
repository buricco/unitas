...
#ifndef lint
static char sccsid[] = "@(#)b.c	4.4 (Berkeley) 4/17/91";
#endif /* not lint */
...
struct fa *cgotofn()
{
	...
	for (i=0; i<spinit; i++) {
		...
		iposns[curpos] = 1;
		cp = point[curpos];
		_dprintf("i = %d, spinit = %d, curpos = %d\n", i, spinit, curpos);
		switch (type(cp)) {
			case CHAR:
				k = (int) right(cp);
				if (isyms[k] != 1) {
	...
	_dprintf("s = %d\n", s, NULL, NULL);
		...
		for (j=0; j<ssmax; j++) {	/* nextstate(s, ssyms[j]) */
			...
			if (notin(state, n, &prev)) {
				if (n >= NSTATES) {
					_dprintf("cgotofn: notin; state = %d, n = %d\n", state, n, NULL);
					overflo();
				}
				state[++n] = add(setcnt);
				_dprintf("	delta(%d,%o) = %d", s,c,n);
				_dprintf(", ind = %d\n", ind+1, NULL, NULL);
				fatab[++ind] = c;
				fatab[++ind] = n;
				numtrans++;
	...
	for (i=0; i<=n; i++) {
		...
		pfa->st = where[0];
		_dprintf("state %d: (%o)\n", i, pfa, NULL);
		_dprintf("	numtrans = %d,	default = %o\n", pfa->cch, pfa->st, NULL);
		for (k=1; k<=pfa->cch; k++) {
			(pfa+k)->st = where[ (int) (pfa+k)->st];
			_dprintf("	char = %o,	nextstate = %o\n",(pfa+k)->cch, (pfa+k)->st, NULL);
		}
	}
	pfa = where[0];
	if ((num = pfa->cch) < 0)
