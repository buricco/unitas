...
static char *e_openfail = "cannot open input file ";
...
loop:
	...
	strncpy(mbuf, e_openfail, sizeof(mbuf)-1);
	strncpy(mbuf, sargv[ifile], sizeof(mbuf)-(1+strlen(e_openfail)));
...
char *s;
{
	...
	if (ss == 0 || *ss == 0)
		...
		(void) fprintf(stderr, "%s on line %d, %s\n",
		    s, ln+1, ss);
	(void) fflush(stderr);
	...
	*/
	...
	for (filecounter = 0; filecounter < argc; filecounter++) {
		if ((strlen(av[filecounter])) >= PATH_MAX) {
			(void) fprintf(stderr, "File argument too long\n");
			exit(2);
		}
	}
