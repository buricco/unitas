...
casediv:
		...
		case 'i':
			...
			c = sbackc(q);
			if (sfbeg(q) == 0)
				error("input base is too large\n");
			if (c < 2)
				error("input base is too small\n");
			if (c > 16)
				error("input base is too large\n");
			release(q);
		...
		case 'k':
			...
			if (c > BC_SCALE_MAX)
				error("scale factor is too large\n");
			release(q);
...
#define	G1	"array %o elt %d odd\n"
#define	G2	"tmps %o p %o\n"
void
garbage(char *s)
{
	int i;
