...
#define	Openerr		"%s: ERROR: Could not open %s: "
#define	Digiterr	"%s: ERROR: argument to %s option must be numeric\n"
#define	Badline		"%s: WARNING: File %s: bad input line %d ignored\n"
...
set_info(char *namep)
{
	...
	while (fgets(buf, sizeof (buf), fp) != NULL) {
		...
		if (ioctl(sadfd, SAD_SAP, &push) < 0) {
			...
			retcode = 1;
			(void) fprintf(stderr,
			    "%s: ERROR: File %s: could not configure "
			    "autopush for line %d\n", Cmdp, namep, line);
			switch (error) {
			case EPERM:
				(void) fprintf(stderr, "%s: ERROR: "
				    "You do not have permission to set autopush "
				    "information\n", Cmdp);
				break;
			...
			case ENOSR:
				(void) fprintf(stderr, "%s: ERROR: Ran "
...
rem_info(major_t maj, minor_t min)
{
	...
	if (ioctl(sadfd, SAD_SAP, &push) < 0) {
		...
		retcode = 1;
		(void) fprintf(stderr, "%s: ERROR: Could not remove "
		    "autopush information\n", Cmdp);
		switch (error) {
		case EPERM:
			(void) fprintf(stderr, "%s: ERROR: You do not "
		...
		case ENODEV:
			(void) fprintf(stderr, "%s: ERROR: Major/minor "
			    "not configured for autopush\n", Cmdp);
			break;

		case ERANGE:
			(void) fprintf(stderr, "%s: ERROR: minor must "
...
get_info(major_t maj, minor_t min)
{
	...
	if (ioctl(sadfd, SAD_GAP, &push) < 0) {
		...
		(void) fprintf(stderr, "%s: ERROR: Could not get "
		    "autopush information\n", Cmdp);
		switch (error) {
		case EINVAL:
			(void) fprintf(stderr, "%s: ERROR: Invalid "
			    "major device number\n", Cmdp);
			break;

		case ENOSTR:
			(void) fprintf(stderr, "%s: ERROR: Major "
	...
	if (push.sap_npush > 1)
		for (i = 0; i < (push.sap_npush - 1); i++)
			(void) printf("%s ", push.sap_list[i]);
	(void) printf("%s\n", push.sap_list[(push.sap_npush - 1)]);
#endif
	return (0);
}
...
parse_line(char *linep, int lineno, char *namep, struct strapush *pushp)
{
	...
	*cp = c;
	...
	if (*cp) {
		(void) fprintf(stderr,
		    "%s: ERROR: File %s: too many modules, line %d "
		    "ignored\n", Cmdp, namep, lineno);
		return (1);
	}
	return (0);
