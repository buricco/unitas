...
main(int argc, char **argv)
{
	...
	if (argc != 2) {
		(void) fprintf(stderr, "Usage: sdiff [-l] [-s] [-o output] [-w #] file1 file2\n");
		return 2;
	}
	...
	if (oflag) {
		if (tempfd == -1) {
			...
			tempfd = open(mktemp(temp), O_WRONLY);
			if (tempfd == -1) {
				error("Cannot open/create temp %s", temp);
				free(temp);
				temp = 0;
			}
		}
		ltemp = strdup("/tmp/sdifflXXXXXX");
		if ((lfd = open(mktemp(ltemp), O_WRONLY) ||
			(left = fdopen(lfd, "w")) == NULL))
				error("Cannot open/create temp %s", ltemp);
		rtemp = strdup("/tmp/sdiffrXXXXXX");
		if ((rfd = open(mktemp(rtemp), O_WRONLY) ||
			(right = fdopen(rfd, "w")) == NULL))
				error("Cannot open/create temp file %s", rtemp);
	...
	/* Process diff cmd */
		...
		default:
			(void) fprintf(stderr, "%c: cmd not found\n", cmd);
			break;
		}
	...
	while (notacc) {
		...
		case 'e':
			...
			case '\n':
				...
				break;
			default:
				(void) fprintf(stderr, "Illegal command %s reenter\n",
					cp);
				break;
			}
			if (notacc == 0)
		...
		default:
			(void) fprintf(stderr, "Illegal command reenter\n");
			break;
		}
	}
}
...
filename(char *pa1, char *pa2)
{
	...
	if (stat(a1, &stbuf) != -1 && ((stbuf.st_mode&S_IFMT) == S_IFDIR)) {
		...
		if (fstat(fileno(stdin), &stbuf) == -1)
			error("Cannot process stdin", NULL);
		pa1 = temp = strdup("/tmp/sdiffXXXXXX");
		if ((tempfd = open(mktemp(temp), O_WRONLY)) == -1 ||
